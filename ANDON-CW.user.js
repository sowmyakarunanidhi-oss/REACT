...
(function() {
    ...
    headerTitle.textContent = '🕰️ACHT BUDDY🕰️';
    ...
    return new Promise((resolve) => {
        ...
        message.innerHTML = `
            <div style="font-weight: bold; font-size: 16px; margin-bottom: 10px;">🔔 New Case Detected</div>
            <div style="margin-bottom: 10px;">Case ID: <span style="color: #00A8E1;">${caseId}</span></div>
            <div>Was this case <strong>auto-assigned</strong> to you?</div>`;
...
function getParagonAvailableTime() {
 ...
 function checkACHTAlert() {
    ...
    console.log('🔍 ACHT Alert Check:', {
        totalTimeSpent: totalTimeSpent,
        totalTimeMinutes: totalTimeSpent / 60,
        currentCaseCount: currentCaseCount,
        achtMinutes: achtMinutes.toFixed(2),
        threshold: 15,
        willBeep: achtMinutes > 15,
        currentMinute: currentMinute,
        isRegularCase: isRegularCaseWindow()
    });
...
function updateACHT() {
    ...
    // ✅ Only check alerts if actively working
    if (!onBreak && !offline) {
        checkACHTAlert();
    }
}
...
if (lastCaseId && lastCaseId !== '' && caseStartTime) {
    ...
    // Check if this case has a pending reject status
    const pendingRejectsKey = `pendingRejects_${new Date().toISOString().split('T')[0]}`;
    let pendingRejects = GM_getValue(pendingRejectsKey, {});
    const status = pendingRejects[lastCaseId] || getCaseStatus();

 // ✅ ADD THIS CHECK - Prevent duplicate entries
    ...
    // Clear the pending reject status after using it
    if (pendingRejects[lastCaseId]) {
       ...
       /* // Save previous case details if exists
        ...
        // ✅ NEW: Ask for confirmation before incrementing
...
function getCaseStatus() {
    ...
    // Check if it's a rejected case (on the resolve page)
    if (url.includes('/hz/action/resolve-case') ||
        url.includes('/hz/api/get-resolve-case-data?caseId=')) {
        return 'Rejected case';
    }
